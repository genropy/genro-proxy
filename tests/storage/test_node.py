...
# In genro-proxy, cloud support is always available (Apache 2.0)
# Check if required cloud dependencies are installed
try:
    import fsspec
    _HAS_FSSPEC = True
except ImportError:
...
@pytest.mark.skipif(_HAS_FSSPEC, reason="Test only valid when fsspec is not installed")
class TestStorageNodeCloudProtocolsWithoutFsspec:
    """Tests that cloud protocols raise NotImplementedError without fsspec.

    These tests are only valid when fsspec is NOT installed.
    When fsspec is installed, cloud protocols are supported.
    """
